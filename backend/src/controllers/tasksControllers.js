...
export const getAllTasks = async (req, res) => {
    ...
    switch (filter) {
        ...
        case "week": {
            ...
            startDate = new Date(now.getFullYear(), now.getMonth(), mondayDate); break;
        }
        case "month": {
...
export const createTasks = async (req, res) => {
    try {
        ...
        console.error("Error when call createTasks: ", error);
        res.status(500).json({
            message: "Server error!"
        })
    }
...
export const updateTasks = async (req, res) => {
    try {
        ...
        console.error("Error when call updateTasks: ", error);
        res.status(500).json({
            message: "Server error!"
        })
    }
...
export const deleteTasks = async (req, res) => {
    try {
        ...
        console.error("Error when call deleteTasks: ", error);
        res.status(500).json({
            message: "Server error!"
        })
    }
